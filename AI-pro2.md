...
## 核心功能 3：学习主题匹配及自习室创建/加入 RoomCreate()、RoomAttend()
...
- 页面交互与执行步骤：
  ...
  3. 加入自习室：
     ...
     2. 点击列表选择，再次点击加入此选择，并校验房间状态与剩余位置。
     3. 校验通过则加入房间，更新房间人数。
  4. 成功后跳转 /study-room/[id] 伴学页面。

## 核心功能 4：隐私化在线伴学 Camera()
- 用户故事：我希望开启视频闭麦伴学，并可使用手部模式/背景模糊保护隐私。
- 页面交互与执行步骤：
  1. 进入自习室房间，申请摄像头权限。
  2. 默认：摄像头开启(背景模糊模式)，麦克风无法开启。
  3. 提供两种模式切换：仅手部 / 背景模糊。
  4. 视频流本地实时处理，不上传原始画面。
  5. 模式选择同步至后端房间成员表。
  6. 可随时开关摄像头，关闭后显示占位图。
